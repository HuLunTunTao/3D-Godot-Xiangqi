...
func test_integrity_error_includes_expected_and_actual() -> void:
	...
	var unknown_size := Loader.integrity_failure_text("", "bbb", -1, 52602089)
	assert_string_contains(unknown_size, "无法读取文件大小")
	assert_string_contains(unknown_size, "52602089")
	assert_false(unknown_size.contains("大小 -1"), unknown_size)
