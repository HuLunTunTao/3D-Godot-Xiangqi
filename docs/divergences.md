...
| D007 | PLATFORM | search thread | C++ multi-thread Workers share racy TT. Addon: one Godot `Thread` on desktop; web export is single-thread so search is a main-thread GDScript coroutine that `await`s `process_frame` between ID depths / root moves (one `start_search` / one `tt.new_search` per think). TT single-writer; results via `call_deferred` (thread) or direct emit (coop). `start_search` defaults to async; `sync:true` for GUT/tools only; `cooperative:true` forces the web yield path in tests. History reused across searches. | `test_addon_search_time.gd` + `test_addon_search_coop.gd` + `run_async_search_test.gd` | accepted |
