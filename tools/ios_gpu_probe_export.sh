#!/usr/bin/env bash
# Build Godot 4.6.1 iOS diagnostic package for GPU canary retest.
# Machine-local helper (see AGENTS.md). Not part of runtime.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAMP="$(date +%Y%m%d_%H%M%S)"
TMP="/tmp/godot-pikafish-ios-probe-${STAMP}"
...
if "run/main_scene" in pg:
    import re
    pg = re.sub(r'(?m)^run/main_scene=.*$', 'run/main_scene="res://src/test/mobile_gpu_probe.tscn"', pg)
else:
    pg = pg.replace("[application]\n", "[application]\nrun/main_scene=\"res://src/test/mobile_gpu_probe.tscn\"\n")
(root / "project.godot").write_text(pg)
PY
...
exclude_filter=""
export_path="export/pikafish_gpu_probe.ipa"
...
"$GODOT461" --headless --path "$TMP" --export-debug "iOS" "$TMP/export/pikafish_gpu_probe.xcodeproj" 2>&1 | tee "$TMP/export.log" || true
